...
def logs(filename):
    ...
    if not re.search('.log',filename):
        print('ERROR: API (logs) : Requested file is not log file {}'.format(filename))
        return jsonify([])

    log_dir = os.path.join(".","logs")
    if not os.path.isdir(log_dir):
        print("ERROR: API (logs) : can not find log directory {}".format(filename))
        return jsonify([])

    log_file = os.path.join(log_dir,filename)
    if not os.path.exists(log_file):
        print("ERROR: API (logs) : can not find log file {}".format(filename))
        return jsonify([])
